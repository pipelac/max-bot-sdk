...
try {
    ...
    // Таймаут, DNS-ошибка, отказ в соединении, ошибка SSL-сертификата и т.д.
}
```

> **Ошибки SSL (cURL error 60)**:  
> Если cURL выдает ошибку проверки SSL-сертификата при обращении к `platform-api2.max.ru`, это означает, что на вашем сервере не установлен корневой сертификат Минцифры России.  
> Для решения установите корневые сертификаты ГУЦ Минцифры в хранилище доверенных сертификатов ОС, либо временно в Dev-среде используйте `$config = ConfigBuilder::create('TOKEN')->withVerifySsl(false)->build();`.
