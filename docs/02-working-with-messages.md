...
$keyboard = KeyboardBuilder::build([
    ...
    [
        ['type' => 'request_geo_location', 'text' => '📍 Отправить геолокацию'],
    ],
]);
```
...
Вы также можете использовать следующие типы кнопок:

```php
$keyboard = KeyboardBuilder::build([
    [
        // Открытие мини-приложения
        ['type' => 'open_app', 'text' => '🎮 Играть'],
        
        // Отправка текстового сообщения в чат от имени пользователя
        ['type' => 'message', 'text' => 'Сказать Привет'],
    ],
    [
        // Копирование текста в буфер обмена (текст указывается в payload)
        ['type' => 'clipboard', 'text' => '📋 Копировать промокод', 'payload' => 'PROMO2026'],
    ],
]);
```
...
- Максимум **30 рядов**
- Максимум **7 кнопок** в ряду (лимит **3 кнопки** в ряду, если в нём используются типы `link`, `open_app`, `request_geo_location` или `request_contact`)
