...
### Добавлено
- `Bot::patchCommands(array $commands)`: метод для работы с `PATCH /me/commands` (обновление команд бота) с поддержкой синтаксиса PHP 5.6+.

### Изменено
- `CurlHttpClient`: дефолтный домен по умолчанию обновлён на `https://platform-api2.max.ru` согласно спецификациям MAX Bot API.
- `docs` & `tests`: обновлена документация и тесты под домен `platform-api2.max.ru` и рекомендации по SSL-сертификатам Минцифры.
