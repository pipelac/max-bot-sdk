...
```php// Отправить текстовое сообщение в чат
$msg = $client->messages()->sendMessage(
    ['text' => 'Привет!'],
    null,     // notify
    $chatId   // ID чата
...
```php// Получить участников чата (PaginatedResult<ChatMember>)
...
```php// Подписаться на обновления (webhook)
...
```php// Загрузить файл одним вызовом (шаги 1+2 автоматически)
...
```php// Ответить на callback с обновлением сообщения и уведомлением
