...
- **`master` (v2.x)** — PHP ≥ 8.1, активная разработка
- **[`1.x`](https://github.com/pipelac/max-bot-sdk/tree/1.x) (v1.x)** — PHP ≥ 5.6, активная разработка
...
- ✅ **Качество кода** — PHPUnit-тесты + PHPStan level 9

## Требования

- PHP ≥ 8.1
- расширения: `curl`, `json`, `mbstring`
- доступ до `platform-api2.max.ru` *(по умолчанию, HTTPS)* или `platform-api.max.ru` *(работает через обратную совместимость)*
...
$config = new Config('YOUR_BOT_TOKEN', 60, 5);
```
...
- `MAX_BOT_LOG_REQUESTS` — логирование запросов (по умолчанию: `true`)
- `MAX_BOT_APP_NAME` — имя приложения (по умолчанию: `MaxBot`)
...
$keyboard = KeyboardBuilder::build([
    // Ряд 1
    [
        ['type' => 'callback', 'text' => 'Кнопка 1', 'payload' => 'callback_1'],
        ['type' => 'callback', 'text' => 'Кнопка 2', 'payload' => 'callback_2'],
    ],
    // Ряд 2
    [
...
$msg = $client->messages()->sendTextWithKeyboard('Выберите:', $chatId, [
    [
...
$client->messages()->editMessage($messageId, ['text' => 'Обновлённый текст']);
...
```php
// Получить чат по публичной ссылке (алиасу)
...
echo $chat->getParticipantsCount();
...
$token = $client->uploads()->uploadFile(UploadType::Image, '/path/to/file.jpg');

// Или пошагово:
$uploadResult = $client->uploads()->getUploadUrl(UploadType::Image);
...
if ($update !== null) {
    match ($update->getUpdateType()) {
        'message_created' => handleMessage($update, $client),
        'message_callback' => handleCallback($update, $client),
        default => null, // Неизвестный тип — игнорируем
    };
}

http_response_code(200);

function handleMessage(\MaxBotSdk\DTO\Update $update, \MaxBotSdk\Client $client): void
{
...
function handleCallback(\MaxBotSdk\DTO\Update $update, \MaxBotSdk\Client $client): void
{
...
try {
    ...
    echo 'Ошибка API [' . $e->getStatusCode() . ']: ' . $e->getDescription();
...
# Статический анализ (PHPStan level 9)
composer analyse
